...
public abstract class BetRoomDatabase extends RoomDatabase {
    ...
    private static volatile BetRoomDatabase INSTANCE;
    private static final int NUMBER_OF_THREADS = 4;
    static final ExecutorService databaseWriteExecutor =
            Executors.newFixedThreadPool(NUMBER_OF_THREADS);

    static BetRoomDatabase getDatabase(final Context context) {
        if (INSTANCE == null) {
            synchronized (BetRoomDatabase.class) {
                if (INSTANCE == null) {
                    INSTANCE = Room.databaseBuilder(context.getApplicationContext(),
                                    BetRoomDatabase.class, "bet_database")
    ...
    private static RoomDatabase.Callback sRoomDatabaseCallback = new RoomDatabase.Callback() {
        ...
        public void onCreate(@NonNull SupportSQLiteDatabase db) {
            ...
            databaseWriteExecutor.execute(() -> {
                ...
                BetDao dao = INSTANCE.betDao();
                dao.deleteAll();

                Bet bet = new Bet("Hello");
                dao.insert(bet);
                bet = new Bet("World");
                dao.insert(bet);
            });
